...
def grafico(f,c):
        ...
        try:
                ...
                axis.plot(z,label='RSSI')
                #axis.plot(x[:self.i], self.dados[:self.i, 1], label='ADC 2')
                axis.legend()
                axis.set_ylabel('RSSI (dBm)')

                '''axis1 = f.add_subplot(211)
                axis1.plot(z,psr,label='LUMINOSIDADE')
                #axis.plot(x[:self.i], self.dados[:self.i, 1], label='ADC 2')
                axis1.legend()
                axis1.set_ylabel('LUMINOSIDADE')
...
raiz.title("MEDIDA DE SIMULAÇÃO RSSI (Radio Signal Strengh Indicator)") #funcao para alterar titulo da janela
